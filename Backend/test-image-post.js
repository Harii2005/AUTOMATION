...
async function scheduleImagePost() {
  try {
    ...
    console.log(`📅 Test schedule time (2 min from now): ${testScheduledTime.toISOString()}`);
...
#ComputerGraphics #BezierCurves #StudyNotes #Programming`;
    ...
    console.log(`🔧 Post options:`, JSON.stringify(scheduledPost.postOptions, null, 2));

    console.log('\n🚀 The automatic posting service will process this post in ~2 minutes!');
    console.log('💡 Make sure your backend server is running to see the post get published.');
