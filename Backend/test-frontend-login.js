...
async function testFrontendLogin() {
  try {
    ...
    try {
      ...
      console.log(
        "Token received:",
        frontendResponse.data.token ? "YES" : "NO"
      );
    } catch (frontendError) {
      console.error(
        "❌ Frontend-style call failed:",
        frontendError.response?.data || frontendError.message
      );
    }
    ...
    try {
      ...
      if (frontendCheck.data.includes("<html")) {
        ...
        if (
          frontendCheck.data.includes("react") ||
          frontendCheck.data.includes("root")
        ) {
        ...
        console.log(
          "❌ No HTML content - frontend might not be built properly"
        );
      }
    } catch (frontendError) {
    ...
    try {
      const loginPageResponse = await axios.get(
        "https://frontenddautomation.onrender.com/login",
        {
          timeout: 10000,
        }
      );
      ...
      const hasEmailInput =
        loginPageResponse.data.includes("email") ||
        loginPageResponse.data.includes("Email");
      const hasPasswordInput =
        loginPageResponse.data.includes("password") ||
        loginPageResponse.data.includes("Password");
      const hasLoginForm =
        loginPageResponse.data.includes("form") ||
        loginPageResponse.data.includes("login");
    ...
    try {
      ...
      console.log(
        "⚠️  CORS preflight check:",
        corsError.response?.status || corsError.message
      );
    }
