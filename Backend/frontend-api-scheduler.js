...
async function scheduleTwitterPostThroughFrontend() {
  try {
    ...
    const tweetContent = "🌐 Frontend Tweet at 4:00 PM! 🚀 Posted through the web interface like a real user! #FrontendAutomation #WebScheduling";
    ...
    try {
      ...
      if (timeUntil4PM > 0) {
        console.log(`⏳ Setting timer for ${Math.floor(timeUntil4PM / 1000 / 60)} minutes until 4:00 PM...`);
        ...
        setTimeout(async () => {
          try {
            ...
            console.error("❌ Error posting 4:00 PM tweet:", scheduleError.response?.data || scheduleError.message);
          }
        }, timeUntil4PM);
        
        console.log("✅ 4:00 PM tweet scheduled successfully!");
        console.log("🔄 Timer is set, script will post at exactly 4:00 PM");
        console.log("🌐 All requests appear to come from frontend:", FRONTEND_URL);
        
        // Keep the script running
        setInterval(() => {
          const currentTime = new Date();
          const minutesLeft = Math.floor((scheduledTime - currentTime) / 1000 / 60);
          if (minutesLeft > 0) {
            console.log(`💓 Frontend scheduler heartbeat: ${minutesLeft} minutes until 4:00 PM tweet`);
          }
        }, 30000);
      ...
      console.error("❌ Error with posting:", postError.response?.data || postError.message);
      ...
      try {
        ...
        console.log("✅ Fallback scheduling successful:", scheduleResponse.data);
      } catch (fallbackError) {
    ...
    console.error("❌ Frontend scheduling error:", error.response?.data || error.message);
  }
}
