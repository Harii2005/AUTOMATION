...
async function makeInstagramPostFixed() {
  ...
  const postData = {
    imageUrl: 'https://images.unsplash.com/photo-1611224923853-80b023f02d71?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1080&q=80',
  ...
  try {
    ...
    try {
      ...
      console.log(`│ Type: ${(postDetails.data.media_type || 'IMAGE').padEnd(43)} │`);
      console.log(`│ Posted: ${new Date(postDetails.data.timestamp).toLocaleString().padEnd(41)} │`);
      console.log('│                                                     │');
      ...
      console.log('✅ Post published successfully! (verification had issues but post is live)');
      return {
        success: true,
        postId: postId,
        details: null
...
if (require.main === module) {
  makeInstagramPostFixed().then((result) => {
    if (result.success) {
      ...
      console.log('📱 Check your Instagram @automationtesting_hari to see the post!');
    } else {
