...
async function testChatEndpoint() {
  try {
    ...
    const response = await axios.post(
      `${baseURL}/api/chat/generate-content`,
      testData,
      {
        headers: {
    ...
    if (error.response) {
      ...
      if (error.response.status === 401) {
        console.log(
          "💡 Note: This error is expected if authentication is required."
        );
        console.log(
          "💡 To test with auth, you'll need to provide a valid JWT token."
        );
      }
    } else {
      ...
      if (error.code === "ECONNREFUSED") {
        ...
        console.log(
          "💡 Try running: npm start or node src/index.js in the Backend directory"
        );
      }
    }
  }
