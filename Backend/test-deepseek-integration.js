...
async function testDeepSeekAPI() {
  try {
    ...
    console.log("DeepSeek API Key present:", process.env.DEEPSEEK_API ? "✅ Yes" : "❌ No");
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a helpful AI assistant. Respond concisely and friendly."
        },
        {
          role: "user",
          content: "Hello! Please respond with a short greeting to confirm you're working."
        }
      ],
      max_tokens: 100,
      temperature: 0.7,
    ...
    const contentCompletion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are a social media content creator. Create engaging professional text posts. Focus on creating valuable, shareable content."
        },
        {
          role: "user",
          content: "Create a motivational post about productivity tips for remote workers"
        }
      ],
      max_tokens: 200,
      temperature: 0.7,
