...
async function checkTwitterAccount() {
  try {
    ...
    if (twitterAccounts.length > 0) {
      ...
      const twitterUppercase = accounts.filter(acc => acc.platform === 'TWITTER');
      if (twitterUppercase.length > 0) {
        console.log('Found Twitter account with uppercase platform name, fixing...');
        ...
        if (updateError) {
          ...
          console.log('✅ Fixed Twitter account platform case:', updatedAccount);
        }
      } else {
