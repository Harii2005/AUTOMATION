...
async function testLiveInstagramAPI() {
  ...
  try {
    ...
    console.log(
      `   Token Active: ${publicResponse.data.credentials.hasAccessToken}\n`
    );
    ...
    try {
      ...
      if (error.response?.status === 401) {
        console.log(
          "✅ Instagram test endpoint exists (authentication required)"
        );
    ...
    try {
      ...
      if (error.response?.status === 401) {
        console.log(
          "✅ Instagram account endpoint exists (authentication required)"
        );
      } else if (error.response?.status === 404) {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        console.log(
          "✅ Instagram test-post endpoint exists (authentication required)"
        );
      } else if (error.response?.status === 404) {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        console.log(
          "✅ Instagram post endpoint exists (authentication required)"
        );
      } else if (error.response?.status === 404) {
