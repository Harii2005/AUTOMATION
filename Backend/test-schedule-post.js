...
async function testSchedulePost() {
  ...
  try {
    ...
    console.log("Social accounts query result:", {
      data: socialAccounts,
      error: accountsError,
    });
    ...
    console.log(
      "Available platforms:",
      socialAccounts.map((account) => account.platform.toLowerCase())
    );
    ...
    for (const platform of platforms) {
      ...
      if (socialError || !socialAccount) {
        console.warn(
          `❌ No connected ${platform} account found for user ${testUserId}`
        );
