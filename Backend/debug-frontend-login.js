  ...
  try {
    ...
    try {
      ...
      await page.waitForSelector(
        'button[type="submit"], button:contains("Sign in"), button:contains("Login")',
        { timeout: 5000 }
      );
    ...
    try {
      await Promise.race([
        ...
        page.waitForSelector('.error, [role="alert"], .alert-error', {
          timeout: 10000,
        }),
      ]);
      ...
      if (currentUrl.includes("dashboard")) {
        ...
        const errorElements = await page.$$(
          '.error, [role="alert"], .alert-error, .text-red-500, .text-danger'
        );
        ...
        console.log(
          "Check the FRONTEND CONSOLE messages above for more details"
        );
      }
    } catch (waitError) {
      console.error(
