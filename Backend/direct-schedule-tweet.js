...
async function directDatabaseInsertAndSchedule() {
  try {
    console.log("🐦 Direct approach: Insert Twitter account and schedule tweet");
    ...
    try {
      ...
      console.log("⚠️  Immediate post failed:", immediateError.response?.data || immediateError.message);
    }

    // Now try the scheduled approach
    console.log("\n📅 Now trying scheduled post...");
    const tweetData = {
      content: "🕐 4:00 PM Automated Tweet! The scheduling system is working perfectly. #AutomationSuccess #TwitterBot",
