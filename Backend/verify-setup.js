...
async function checkUsersWithTwitterAccounts() {
  ...
  try {
    ...
    console.log(
      `✅ Found ${users?.length || 0} users with active Twitter accounts`
    );

    if (users && users.length > 0) {
      users.forEach((user) => {
        console.log(
          `  👤 ${user.email}: @${user.social_accounts[0].accountName}`
        );
      });
    } else {
      console.log(
        "💡 No users with Twitter accounts found. Connect a Twitter account to test posting."
      );
    }
...
async function runFullVerification() {
  ...
  if (passedChecks === totalChecks) {
    console.log(
      "🎉 All checks passed! The automatic posting system is ready to use."
    );
    console.log("💡 You can now:");
    console.log("   1. Schedule posts using the frontend or API");
    console.log(
      '   2. Run "node test-automatic-posting.js" to test the system'
    );
    console.log(
      '   3. Start the server with "npm run dev" to enable automatic posting'
    );
  } else {
    console.log(
