...
async function createTestPost(userId, socialAccountId, delayMinutes = 1) {
  ...
  console.log(`✅ Created test post: ${post.id} scheduled for ${scheduledTime.toISOString()}`);
  return post;
}
...
async function monitorPostStatus(postId, maxWaitMinutes = 5) {
  console.log(`👀 Monitoring post ${postId} for up to ${maxWaitMinutes} minutes...`);
  ...
  while (Date.now() - startTime < maxWaitTime) {
    ...
    console.log(`📊 Post ${postId} status: ${post.status} (${new Date().toLocaleTimeString()})`);
  ...
  console.log(`⏰ Timeout: Post ${postId} did not complete within ${maxWaitMinutes} minutes`);
  return false;
}
...
async function testAutomaticPosting() {
  ...
  try {
    ...
    const testPost = await createTestPost(user.id, user.social_accounts[0].id, 1);
    if (!testPost) {
      return;
    }
...
async function testScheduledPostsQuery() {
  ...
  posts?.forEach(post => {
    console.log(`  - Post ${post.id}: "${post.content.substring(0, 50)}..." scheduled for ${post.scheduledTime}`);
  });
}
...
if (require.main === module) {
  ...
  if (testType === 'query') {
    ...
    console.log('  posting: Test the full automatic posting workflow (default)');
